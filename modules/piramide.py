...
class Piramide:
    ...
    def __init__(self, base_longitud, altura):
        if base_longitud <= 0:
            raise ValueError("La base debe ser positivo")
        self.__base_longitud = base_longitud
        if altura <= 0:
            raise ValueError("La altura debe ser positivo")
    ...
    def volumen(self):
        volumen = ((self.__base_longitud**2) * self.__altura)/3
        return volumen
    ...
    def area_superficial(self):
        superficie = self.__base_longitud * (self.__base_longitud + (4*(self.__altura**2) + (self.__base_longitud/2)**2))
        return superficie
